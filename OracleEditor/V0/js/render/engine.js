...
function drawFaceContent(ctx, face) {
  if (face.imageRaw && face.imageNatural) {
    ...
    if (img && img.complete) {
      // Le filtre est appliqué sur un canvas hors-écran séparé, puis collé sans
      // filtre actif à l'intérieur du clip ci-dessous : évite le bug de rendu où
      // ctx.filter et ctx.clip combinés sur le même contexte peuvent faire
      // ignorer le clip par le moteur de rendu.
      const off = document.createElement('canvas');
      off.width = CARD_W;
      off.height = HALF_H;
      const octx = off.getContext('2d');
      octx.filter = `brightness(${face.filters.brightness}%) contrast(${face.filters.contrast}%) saturate(${face.filters.saturation}%)`;
      if (face.flipH || face.flipV) {
        octx.save();
        if (face.flipH) { octx.translate(CARD_W, 0); octx.scale(-1, 1); }
        if (face.flipV) { octx.translate(0, HALF_H); octx.scale(1, -1); }
        octx.drawImage(img, geo.x, geo.y, geo.drawW, geo.drawH);
        octx.restore();
      } else {
        octx.drawImage(img, geo.x, geo.y, geo.drawW, geo.drawH);
      }
      ctx.save();
      // Marge alignée sur la 3e (dernière) ligne de bordure dessinée dans
      // drawOrnaments (inset 35) + une petite marge de sécurité : l'image
      // s'arrête avant toute la zone ornementale, jamais en dessous.
      const m = 38, r = 10;
      ctx.beginPath();
      ctx.moveTo(m + r, m);
  ...
  const rightBadgeR = 27;
  const rightBadgeCX = CARD_W - 24 - rightBadgeR;
  const rightBadgeCY = 56;
  const bannerH = 50;
  const bannerY0 = rightBadgeCY - bannerH / 2;
  const bannerX0 = 15;
  const bannerX1 = rightBadgeCX - rightBadgeR - 16;
  const bannerW = bannerX1 - bannerX0;

  // Bandeau : part de la bordure gauche, s'arrête avant le badge de droite
  ctx.save();
  ...
  // Badge type : seul icône en cercle, à droite
  ctx.save();
  ctx.beginPath();
  ctx.arc(rightBadgeCX, rightBadgeCY, rightBadgeR + 3, 0, Math.PI * 2);
  ctx.strokeStyle = 'rgba(230,224,207,0.5)';
  ctx.lineWidth = 1.6;
  ctx.stroke();
  ctx.beginPath();
  ctx.arc(rightBadgeCX, rightBadgeCY, rightBadgeR, 0, Math.PI * 2);
  ctx.fillStyle = COLORS.ink;
  ctx.fill();
  ctx.strokeStyle = COLORS.brass;
  ctx.lineWidth = 2.4;
  ...
  ctx.lineWidth = 2.2;
  TYPE_ICONS[face.typeIcon].draw(ctx, rightBadgeCX, rightBadgeCY, rightBadgeR * 0.52);
  ctx.restore();

  // Icône catégorie : flottante, sans cercle, superposée au coin/bandeau
  ctx.save();
  ...
  ctx.fillStyle = COLORS.parchment;
  ctx.lineWidth = 3;
  CATEGORY_ICONS[face.categoryIcon].draw(ctx, 38, 38, 30);
  ctx.restore();
  ...
  if (face.description && face.description.trim()) {
    ...
    const targetHeight = HALF_H - 130;

    do {
      ctx.font = `italic 400 ${descFontSize}px "Iowan Old Style", "Palatino Linotype", Georgia, serif`;
      lines = wrapText(ctx, face.description, boxW - padding * 2);
      lineHeight = descFontSize * 1.32;
      const h = lines.length * lineHeight + padding * 2;
      if (h <= targetHeight || descFontSize <= 13) break;
      descFontSize -= 1;
    } while (true);

    // Le cadre grandit pour accueillir tout le texte (retour à la ligne), jamais de troncature
    const boxH = lines.length * lineHeight + padding * 2;
    const boxY = boxH <= targetHeight ? 128 + (targetHeight - boxH) / 2 : 128;
